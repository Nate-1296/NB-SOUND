...
def ahora_utc_iso() -> str:
    """Marca de tiempo UTC en el formato canónico de los backups.

    Expone :func:`_ahora_iso` para que el modelo Qt registre `backup_ultimo`
    con el mismo formato que escribe el manifest, manteniendo un único origen
    de verdad para la fecha.
    """
    return _ahora_iso()


def _parsear_iso_utc(valor: Optional[str]) -> Optional[datetime]:
    """Parsea una marca ISO a datetime UTC consciente de zona horaria.

    Acepta el formato canónico con sufijo ``Z`` y variantes ISO-8601. Devuelve
    ``None`` si el valor está vacío o no es parseable (se interpreta como
    "nunca se hizo una copia").
    """
    if not valor:
        return None
    txt = str(valor).strip()
...
def backup_programado_vencido(
    frecuencia_dias: int,
    ultimo_iso: Optional[str],
    ahora: Optional[datetime] = None,
) -> bool:
    """Decide si toca crear una copia automática. Función pura y testeable.

    Reglas:
      * ``frecuencia_dias <= 0``  → copias automáticas desactivadas → ``False``.
      * ``ultimo_iso`` vacío o no parseable → nunca se respaldó → ``True``.
      * En otro caso → ``True`` si transcurrieron al menos ``frecuencia_dias``
        desde la última copia.

    `ahora` se inyecta en los tests; por defecto usa la hora UTC actual. Una
    marca sin zona horaria se interpreta como UTC.
    """
    if frecuencia_dias <= 0:
        return False
    ultimo = _parsear_iso_utc(ultimo_iso)
    if ultimo is None:
        return True
    if ahora is None:
    ...
    return (ahora - ultimo) >= timedelta(days=frecuencia_dias)
...
def crear_backup(destino: Path, *, incluir_assets: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="nbsound_backup_") as td:
        ...
        if zip_tmp.exists():
            zip_tmp.unlink()
        try:
            total_assets = _crear_zip_backup(
                zip_tmp, db_tmp, assets_dir, checksums, contenido,
            )
            zip_tmp.replace(destino)
        except Exception:
            # No dejar restos .tmp medio escritos si algo falla (disco lleno,
            # permisos…): se limpia y se propaga para que la UI lo informe.
            try:
                if zip_tmp.exists():
                    zip_tmp.unlink()
            except OSError:
...
def _crear_zip_backup(zip_tmp, db_tmp, assets_dir, checksums, contenido) -> int:
    """Escribe el ZIP del backup (BD + assets + manifest). Devuelve el nº de
    assets incluidos. Aislado para poder limpiar el .tmp ante cualquier fallo
    en :func:`crear_backup`."""
    total_assets = 0
