...
PLAYLIST_AUTO_THIS_IS_MIN_PISTAS = 50
...
def buscar(termino: str, limite: int = 50) -> dict:
    ...
    pistas_filas = list(filas_por_id.values())
...
def estadisticas_extras_perfil() -> dict:
    ...
    dias_activos_fila = obtener_una_fila(
        """
        SELECT COUNT(DISTINCT date(reproducido_en)) AS dias
        FROM historial
        WHERE reproducido_en >= datetime('now', '-30 days')
        """
    )
...
def _reemplazar_pistas_playlist(conexion, playlist_id: int, pista_ids: list[int]) -> None:
    vistos: set[int] = set()
    ...
    for pista_id in pista_ids:
        ...
        if len(ids_limpios) >= PLAYLIST_AUTO_MAX_PISTAS:
            break
...
def _sincronizar_playlist_favoritos() -> int:
    ...
    with transaccion() as conexion:
        ...
        _reemplazar_pistas_playlist(conexion, playlist_id, [int(f["id"]) for f in filas])
...
def _candidatos_this_is(artista_id: int) -> list[int]:
    filas = obtener_filas(
        """
        SELECT
            p.id,
        ...
        LIMIT ?
        """,
        (artista_id, PLAYLIST_AUTO_MAX_PISTAS * 4),
    )
    ids: list[int] = []
    diferidos: list[int] = []
...
def _specs_playlists_automaticas() -> list[dict]:
    ...
    artistas = [
        ...
        if int(artista.get("total_pistas") or 0) >= PLAYLIST_AUTO_THIS_IS_MIN_PISTAS
    ]
    for artista in artistas:
...
def _aplicar_spec_playlist_automatica(spec: dict, *, forzar: bool = False, respetar_cooldown: bool = True) -> dict:
    ...
    candidatos = candidatos[:PLAYLIST_AUTO_MAX_PISTAS]
    ahora = _ahora_iso()
    with transaccion() as conexion:
        if existente:
            playlist_id = int(existente["id"])
            ...
            creada = True
        _reemplazar_pistas_playlist(conexion, playlist_id, candidatos)
    actualizar_portada_playlist_si_cambio(playlist_id)
...
def generar_playlists_inteligentes(limite_creacion: int = 4) -> dict:
    ...
    if creadas:
        ...
        hay_artistas_cerca = any(
            int(artista.get("total_pistas") or 0) >= 35
            for artista in conteos_artistas_para_playlists()
        )
        mensaje = (
            "Cuando tengas más canciones de un artista, aparecerán playlists tipo This is."
            if hay_artistas_cerca
            else "Cuando escuches más canciones se podrán preparar nuevas playlists."
        )
    return {
        "ok": True,
        "creadas": creadas,
...
def reordenar_playlist(playlist_id: int, pista_id: int, nueva_posicion: int) -> dict:
    ...
    with transaccion() as conexion:
        _reemplazar_pistas_playlist(conexion, int(playlist_id), ids)
        _marcar_editada_si_corresponde(conexion, playlist)
...
def reordenar_playlist_completa(playlist_id: int, lista_ids: list[int]) -> dict:
    ...
    with transaccion() as conexion:
        _reemplazar_pistas_playlist(conexion, int(playlist_id), nuevos)
        _marcar_editada_si_corresponde(conexion, playlist)
    return {"ok": True, "mensaje": "Orden actualizado.", "playlist_id": int(playlist_id)}
...
def duplicar_playlist(playlist_id: int, nombre: str = "") -> dict:
    ...
    with transaccion() as conexion:
        _reemplazar_pistas_playlist(conexion, nuevo_id, ids)
...
def buscar_pistas_para_playlist(query: str, playlist_id: Optional[int] = None, limite: int = 100) -> list[dict]:
    ...
    if termino:
        resultados = buscar(termino, limite=limite * 2).get("pistas", [])
    else:
        resultados = listar_pistas(orden="reciente", limite=limite * 2)
