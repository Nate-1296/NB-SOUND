...
class ReproductorSesionDj:
    ...
    def __init__(
        ...
        *,
        ...
        # Reanudacion diferida (persistencia de sesion DJ): cuando se restaura
        # una sesion al reabrir la app, dejamos la pista/offset "preparados" en
        # PAUSADO sin tocar audio; el primer play() los consume. Imita al
        # `_reanudar_seg_pendiente` del reproductor global.
        self._reanudacion_pendiente: bool = False
        self._offset_reanudacion_seg: float = 0.0
    ...
    def play(self) -> bool:
        ...
        with self._lock:
            if not self._pistas:
                return False
            # Sesion restaurada (PAUSADO sin audio cargado): el primer play
            # arranca la pista guardada y aplica el seek diferido.
            if self._reanudacion_pendiente:
                return self._iniciar_reanudacion_pendiente_locked()
    ...
    def buscar_posicion_global(self, seg_global: float) -> bool:
        ...
        with self._lock:
            if not self._pistas:
                return False
            indice_objetivo, offset_local = self._localizar_global_locked(seg_global)
    ...
    def _localizar_global_locked(self, seg_global: float) -> tuple[int, float]:
        """Convierte una posicion absoluta del timeline a (indice, offset_local).

        Devuelve ``(-1, 0.0)`` si la posicion cae mas alla del final de la
        sesion. Debe llamarse con `self._lock` tomado.
        """
        ...
        for idx, p in enumerate(self._pistas):
            ...
            if dur <= 0:
                continue
            if objetivo < acum + dur:
                return idx, max(0.0, objetivo - acum)
            acum += dur
        return -1, 0.0

    def preparar_reanudacion(
        self, pista_id: int, offset_local_seg: float, indice_fallback: int = 0
    ) -> bool:
        """Deja la sesion lista para reanudar en (pista, offset) al primer play.

        NO toca audio: posiciona el indice, anota el offset diferido y deja el
        estado en PAUSADO. El primer :meth:`play` arranca esa pista y consume
        el seek. Busca la pista por ``pista_id`` (robusto a reordenamientos o a
        archivos filtrados al recargar); si no existe, usa ``indice_fallback``.

        Pensado para la restauracion de sesion al reabrir la app (imita
        `_restaurar_pista_activa_persistida` del reproductor global).
        """
        with self._lock:
            if not self._pistas:
                return False
            idx = -1
            if pista_id:
                for i, p in enumerate(self._pistas):
                    if p.pista_id == int(pista_id):
                        idx = i
                        break
            if idx < 0:
                idx = max(0, min(int(indice_fallback), len(self._pistas) - 1))
            self._indice_actual = idx
            self._offset_reanudacion_seg = max(0.0, float(offset_local_seg))
            self._reanudacion_pendiente = True
            self._estado = EstadoSesion.PAUSADO
            self._emitir_estado()
            self._emitir_pista_cambio(idx)
            return True

    def _iniciar_reanudacion_pendiente_locked(self) -> bool:
        """Arranca una sesion restaurada: reproduce la pista preparada y aplica
        el seek diferido (offset dentro de la pista). Consumo unico del estado
        dejado por :meth:`preparar_reanudacion`. Debe llamarse con el lock.
        """
        self._reanudacion_pendiente = False
        offset = max(0.0, float(self._offset_reanudacion_seg))
        self._offset_reanudacion_seg = 0.0
        if not (0 <= self._indice_actual < len(self._pistas)):
            self._indice_actual = 0
        ok = self._reproducir_pista_actual_locked()
        if not ok:
        ...
        self._iniciar_hilo_polling_locked()
        pista = self.pista_actual
        if offset > 0.5 and pista is not None and pista.duracion_seg > 0:
            porc = max(0.0, min(1.0, offset / pista.duracion_seg))
            threading.Thread(
                target=self._aplicar_seek_diferido_locked,
                args=(porc,), daemon=True,
            ).start()
    ...
    def _detener_interno(self, *, notificar: bool) -> None:
        ...
        self._estado = EstadoSesion.DETENIDO
        self._transicion_activa = None
        # Cualquier detencion invalida una reanudacion preparada pero no
        # consumida (p.ej. recargar la sesion descarta el seek pendiente).
        self._reanudacion_pendiente = False
        self._offset_reanudacion_seg = 0.0
        self._stop_event.set()
        self._hilo = None
    ...
    def _saltar_a_locked(self, indice: int, *, hard_cut: bool = False) -> bool:
        ...
        if not ok:
            return False
        # `_saltar_a_locked` solo lo invocan los saltos manuales del usuario
        # (anterior/siguiente/saltar_a): siempre dejan la sesión REPRODUCIENDO.
        # Antes conservaba PAUSADO, lo que dejaba el deck sonando pero el estado
        # en "pausado" (audio sin coincidir con la UI; agravado al readquirir
        # ownership desde el global).
        self._estado = EstadoSesion.REPRODUCIENDO
        self._emitir_estado()
        self._emitir_pista_cambio(self._indice_actual)
        self._iniciar_hilo_polling_locked()
    ...
    def _iniciar_transicion_locked(self, overlap: float, tecnica: str, plan: Optional[TransicionPlan]) -> None:
        ...
        self._transicion_activa = {
            # inicio_ts se fija al PRIMER tick en que el deck entrante ya
            # produce audio (ver _tick_transicion_locked). Mantenerlo None al
            # arrancar evita que el fade corra mientras VLC aún buferea/parsea
            # la pista entrante — esa ventana producía el bajón/silencio que se
            # oía en cada transición.
            "inicio_ts":   None,
        ...
        self._emitir_estado()
        logger.info(
            "DJ transición preparada: %d→%d técnica=%s overlap=%.1fs mix_in_b=%.1fs "
            "stems=%s (dur_b=%.0fs).",
            self._indice_actual, siguiente_idx, tecnica, float(max(0.05, overlap)),
            mix_in_b_seg,
            bool(plan_mezcla.ruta_audio_b_override) if plan_mezcla else False,
            siguiente.duracion_seg,
        )
    ...
    def _tick_transicion_locked(self, pos_seg: float, dur_seg: float) -> None:
        ...
        # ── Gate de arranque del fade ────────────────────────────────────────
        # No empezamos a bajar el volumen del deck saliente hasta que el deck
        # entrante esté REALMENTE sonando (VLC tarda ~100-300 ms en abrir el
        # media; si además hay un seek de mix_in, esperamos a que se aplique).
        # Mientras tanto, mantenemos A a volumen pleno y B en silencio (vol 0):
        # así no hay "bajón → silencio → entra bajo" en cada cambio de pista.
        # `is_playing()` es señal fiable de "ya produce audio" (más que get_time,
        # que VLC puede dejar en 0 un rato aunque el deck arranque).
        if info.get("inicio_ts") is None:
            b_listo = False
            if deck_b_entrando is not None:
                try:
                    tocando = bool(deck_b_entrando.is_playing())
                except Exception:
                    tocando = False
                try:
                    b_ms = deck_b_entrando.get_time()
                except Exception:
                    b_ms = -1
                b_pos_seg = (b_ms / 1000.0) if (b_ms is not None and b_ms > 0) else -1.0
                mix_in_b = float(info.get("mix_in_b") or 0.0)
                if mix_in_b > 0.5:
                    # Esperar también a que el seek de mix_in se haya aplicado.
                    b_listo = tocando and b_pos_seg >= (mix_in_b - 0.5)
                else:
                    b_listo = tocando and b_pos_seg >= 0.0
            espera = time.monotonic() - float(info.get("armado_ts") or 0.0)
            if not b_listo and espera < 2.5:
                # Aún buffereando: A pleno, B en silencio. No avanzamos el fade.
                if deck_a_actual is not None:
                    try: deck_a_actual.audio_set_volume(int(self._volumen))
                    except Exception: pass
                if deck_b_entrando is not None:
                    try: deck_b_entrando.audio_set_volume(0)
                    except Exception: pass
                return
            # Deck entrante listo (o timeout de seguridad): arranca el fade ya.
            info["inicio_ts"] = time.monotonic()
            logger.info(
                "DJ transición: fade arranca (técnica=%s, overlap=%.1fs, "
                "espera_buffer=%.0fms, b_listo=%s).",
                tecnica, overlap, espera * 1000.0, b_listo,
            )
