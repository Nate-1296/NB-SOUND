...
def alternar_bloqueo_pista(sesion_id: int, posicion: int, bloqueada: bool) -> None:
    sql = """
        UPDATE dj_pistas_sesion SET bloqueada=? WHERE sesion_id=? AND posicion=?
    """
    ejecutar(sql, (1 if bloqueada else 0, int(sesion_id), int(posicion)))
