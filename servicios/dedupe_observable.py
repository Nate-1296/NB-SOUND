...
class ServicioDedupeObservable:
    ...
    def _construir_grupos(self, resultado: ResultadoDedupeObservable) -> dict:
        """Agrupa las pistas de biblioteca por clave observable.

        Devuelve un dict clave_observable -> lista de filas (sqlite3.Row). Solo
        incluye grupos con 2+ miembros candidatos. La duracion se valida por par
        en :meth:`_resolver_grupo`, no aqui, porque un grupo puede contener
        pistas con la misma clave de texto+portada pero distinta duracion (no
        necesariamente todas duplicadas entre si).
        """
        try:
            filas = obtener_filas(
        ...
        for fila in filas:
            ...
            portada_ruta = fila["portada_ruta"]
            if portada_ruta:
                rp = str(portada_ruta)
                if rp not in cache_portada:
                    cache_portada[rp] = hash_portada(rp)
                portada_h = cache_portada[rp]
            else:
                portada_h = None
            clave = clave_observable(
                fila["titulo"], fila["artista_nombre"], fila["album_titulo"], portada_h
            )
            if clave is None:
                continue
        ...
        *,
        ...
        for fila in perdedores:
            razon = (
                ...
                f"dur={fila['duracion_seg']}~{dur_conservar} "
                f"portada={str(clave[3])[:12]}) policy={DUPLICATE_POLICY}"
            )
