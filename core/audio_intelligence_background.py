...
class AudioIntelligenceBackgroundService:
    ...
    def retry_failed(self, *, run_id: str | None = None) -> dict:
        ...
        retryable = obtener_una_fila(
            ...
            WHERE job_type='deep' AND status='failed' AND attempts < ?
            """,
            (cfg.max_attempts,),
        )["c"]
        ...
        ejecutar(
            ...
            WHERE job_type='deep' AND status='failed' AND attempts < ?
            """,
            (run_id, cfg.max_attempts, cfg.max_attempts),
        )
        self._set_run_status(run_id, "pending")
        return self.status(run_id=run_id, message="Jobs deep fallidos listos para reintento.")
