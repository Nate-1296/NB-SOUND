...
from db.conexion import ejecutar, obtener_filas, obtener_una_fila
...
class ImportRecoveryService:
    ...
    def retry_assets_missing(self, *, kinds: set[str] | None = None, limit: int = 0, clear_negative_cache: bool = True) -> dict:
        ...
        portadas_enlazadas = self.backfill_portadas_album()
        ...
        print(f"Portadas de álbum enlazadas (portada_ruta): {portadas_enlazadas}\n")
        ...
        return {
            **self.status(),
            ...
            "skipped": skipped,
            "failed": failed,
            "portadas_enlazadas": portadas_enlazadas,
            "action": "retry_assets_missing",
        }
    ...
    def retry_deep_failed(self) -> dict:
        ...
        # El reintento deep es asíncrono: `retry_failed` reencola los jobs y el
        # procesamiento ocurre en el worker de background. Exponemos cuántos se
        # reencolaron (`requeued`) para que la UI muestre un mensaje veraz en
        # vez de "procesados=0" (que parecía indicar que no hizo nada).
        requeued = int((snapshot or {}).get("requeued") or 0)
        return {
            **self.status(),
            "deep_snapshot": snapshot,
            "requeued": requeued,
            "action": "retry_deep_failed",
        }
