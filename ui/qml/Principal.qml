...
ApplicationWindow {
    ...
    //
    // Secuencia robusta:
    //   1. `preparar_cierre()` corta el audio inmediatamente (aunque algún
    //      teardown async tarde) PERO conserva la pista/cola/posición y las
    //      persiste, para reanudar la sesión al reabrir. Antes se usaba
    //      `detener_forzado()`, que vaciaba la cola y limpiaba la pista justo
    //      antes del guardado de `cerrar()` — rompía la persistencia (#0).
    ...
    function saltar_segundos(delta) {
        var dur = Number(reproductor.duracion_seg) || 0
        if (dur <= 0)
            return
        var destino = Math.max(0, Math.min(dur, (Number(reproductor.posicion_seg) || 0) + delta))
        reproductor.buscar_posicion(destino)
    }
    ...
    // Flechas izquierda/derecha: retroceder/avanzar 10s dentro de la pista.
    Shortcut { sequence: "Left"; enabled: ventana_principal._atajos_habilitados(); onActivated: ventana_principal.saltar_segundos(-10) }
    Shortcut { sequence: "Right"; enabled: ventana_principal._atajos_habilitados(); onActivated: ventana_principal.saltar_segundos(10) }
    // Flechas arriba/abajo: pista anterior/siguiente (cambio de canción).
    Shortcut { sequence: "Up"; enabled: ventana_principal._atajos_habilitados(); onActivated: reproductor.anterior() }
    Shortcut { sequence: "Down"; enabled: ventana_principal._atajos_habilitados(); onActivated: reproductor.siguiente() }
    ...
    Shortcut { sequence: "K"; enabled: ventana_principal._atajos_habilitados() && reproductor.karaoke_disponible; onActivated: reproductor.alternar_karaoke() }
    Shortcut { sequence: "D"; enabled: ventana_principal._atajos_habilitados(); onActivated: reproductor.sorprenderme() }
    ...
    onFuente_uiChanged: _actualizar_delta_tipografia()

    // Reduce 2 px SOLO si la fuente de UI es monoespaciada (todas las mono del
    // sistema contienen "mono"; añadimos otras comunes). Con fuente
    // proporcional queda en tamaño normal.
    function _actualizar_delta_tipografia() {
        var f = String(ventana_principal.fuente_ui || "").toLowerCase()
        var claves = ["mono", "consolas", "courier", "menlo", "monaco"]
        var esMono = false
        for (var i = 0; i < claves.length; i++) {
            if (f.indexOf(claves[i]) >= 0) { esMono = true; break }
        }
        Componentes.UiTokens.fontSizeDelta = esMono ? -2 : 0
    }

    Component.onCompleted: {
        _actualizar_delta_tipografia()
