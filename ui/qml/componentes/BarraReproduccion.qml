...
Rectangle {
    ...
    // El slot del botón de karaoke se reserva SIEMPRE en la rejilla de
    // utilidades. La disponibilidad del karaoke es por-pista, así que colapsar
    // su celda (visible) desplazaba el resto de botones al cambiar de pista.
    // Con el slot fijo, el botón solo aparece/desaparece (opacity) sin mover nada.
    readonly property bool karaoke_visible: true
    readonly property int columnas_utilidades: layout_compacto ? 4 : (karaoke_visible ? 8 : 7)
    ...
    property real ancho_utilidades: layout_compacto ? 150 : (layout_medio ? (karaoke_visible ? 444 : 400) : (karaoke_visible ? 472 : 428))
    property real ancho_utilidades_min: layout_compacto ? 146 : (layout_medio ? (karaoke_visible ? 436 : 392) : (karaoke_visible ? 464 : 420))
        ...
        Item {
            ...
            GridLayout {
                ...
                BtnControl {
                    ...
                    Layout.row: 0
                    Layout.column: raiz.karaoke_visible ? 1 : 0
                    onClicked: _accionSorprenderme()
                }

                BtnControl {
                    id: boton_karaoke
                    // Slot siempre reservado (ver karaoke_visible): no se colapsa
                    // la celda; el botón se atenúa e inhabilita cuando la pista
                    // actual no tiene karaoke, sin desplazar al resto.
                    opacity: reproductor.karaoke_disponible ? 1 : 0
                    enabled: reproductor.karaoke_disponible
                ...
                // Añadir a playlist (menú de 3 puntos). Antes vivía a la
                // izquierda, junto al título; ahora va con el resto de
                // utilidades de la derecha, entre Lyrics y Cola. En layout
                // compacto se oculta para no romper la rejilla de 2 filas
                // (la barra ya es estrecha); sigue accesible desde los menús
                // de pista en Biblioteca/Búsqueda/Playlist.
                BtnControl {
                    visible: !raiz.layout_compacto
                    iconSource: "../assets/icons/more-vertical.svg"
                    enabled: hay_pista_activa
                    Layout.row: 0
                    Layout.column: raiz.karaoke_visible ? 3 : 2
                    onClicked: menuAgregarPlaylist.abrir(
                        pista_activa && pista_activa.id ? pista_activa.id : -1,
                        titulo_seguro)
                }
                ...
                BtnControl {
                    ...
                    activo: cola_visible
                    Layout.row: 0
                    Layout.column: raiz.layout_compacto ? (raiz.karaoke_visible ? 3 : 2)
                                                        : (raiz.karaoke_visible ? 4 : 3)
                ...
                Item {
                    ...
                    Layout.column: raiz.layout_compacto ? 0 : (raiz.karaoke_visible ? 5 : 4)
                ...
                BtnControl {
                    ...
                    Layout.row: raiz.layout_compacto ? 1 : 0
                    Layout.column: raiz.layout_compacto ? 2 : (raiz.karaoke_visible ? 6 : 5)
                    onClicked: {
                ...
                BtnControl {
                    ...
                    Layout.column: raiz.layout_compacto ? 3 : (raiz.karaoke_visible ? 7 : 6)
                    onClicked: {
    ...
    MenuAgregarPlaylist {
        ...
        tema: raiz.tema
        onGuardado: function(mensaje) {
            if (shell && shell.mostrar_toast_global)
                shell.mostrar_toast_global(mensaje, "info")
        }
    }
