...
Rectangle {
    ...
    function altoTarjeta(ancho) {
        if (modo_vista_efectivo === "lista")
            return 86
        if (modo_vista_efectivo === "grid-sm")
            return Math.max(206, Math.min(228, ancho + 46))
        if (modo_vista_efectivo === "grid-lg")
            return Math.max(262, Math.min(320, ancho + 52))
        return Math.max(238, Math.min(276, ancho + 48))
    }
    ...
    Component {
        ...
        Rectangle {
            ...
            width: anchoTarjeta(playlistsFlow.width)
            height: altoTarjeta(width)
            radius: UiTokens.radiusSm
            clip: true
            ...
            ColumnLayout {
                ...
                CoverBox {
                    Layout.fillWidth: true
                    Layout.preferredHeight: Math.max(118, Math.min(card.width - UiTokens.spacing20, card.height - 82))
                    Layout.maximumHeight: Math.max(118, card.height - 82)
                    portada: card.plData.portada_ruta || ""
                    coverRadius: UiTokens.radiusSm
