...
Rectangle {
    ...
    // Portada de la pista actual (mismo origen que la barra global). Se
    // reevalúa al cambiar de pista (_idx → notify pistaActualCambiada).
    readonly property string _portada_actual: {
        if (raiz._idx < 0) return ""
        var ruta = djPrivado.dj_portada_pista(raiz._idx)
        ...
        ColumnLayout {
            ...
            AppCard {
                ...
                RowLayout {
                    ...
                    // Portada pequeña de la pista actual (acerca el look al
                    // reproductor global). Placeholder genérico si no hay.
                    Rectangle {
                        ...
                        Image {
                            id: dj_portada_placeholder
                            anchors.centerIn: parent
                            width: 22; height: 22
                            visible: raiz._portada_actual === "" || dj_portada_img.status === Image.Error
                        ...
                        MultiEffect {
                            visible: dj_portada_placeholder.visible
                            anchors.fill: dj_portada_placeholder
                            source: dj_portada_placeholder
                            colorization: 1.0
                            colorizationColor: raiz.tema.textoMuted
                        }
                        Image {
                            id: dj_portada_img
                            anchors.fill: parent
                            visible: raiz._portada_actual !== "" && status !== Image.Error
                            source: raiz._portada_actual
                ...
                RowLayout {
                    ...
                    // Transporte + volumen alineados a la izquierda; "Cerrar
                    // sesión" empujado a la derecha por el spacer flexible.
                    BotonIconoDj {
                    ...
                    // Control de volumen de la sesión DJ (mismo patrón que el
                    // reproductor global: icono + deslizador), junto al transporte.
                    Item {
                        ...
                        MultiEffect {
                            anchors.fill: icon_vol_dj
                            source: icon_vol_dj
                            colorization: 1.0
                            colorizationColor: raiz.tema.textoSec
                        }
                    }
                    Item {
                        ...
                        SliderLine {
                            anchors.fill: parent
                            tema: raiz.tema
                            ratio: Math.max(0, Math.min(1, djPrivado.dj_volumen / 100))
                            live: true
                            visualHeight: 5
                            handleBaseSize: 10
                            handleActiveSize: 14
                            onPreviewed: function(r) { djPrivado.dj_set_volumen(Math.round(r * 100)) }
                            onCommitted: function(r) { djPrivado.dj_set_volumen(Math.round(r * 100)) }
                        }
                    }
                    // Empuja "Cerrar sesión" a la derecha.
            ...
            AppCard {
                ...
                RowLayout {
                    ...
                    // Se eliminaron "Crear variante" y "Alargar 15 min": la
                    // primera duplicaba a Regenerar; la segunda no se reflejaba
                    // y la cola ya se extiende sola en background
                    // (continuar_construccion). La sesión se gestiona con
                    // Guardar/Abrir playlist.
                    Rectangle {
                        // Si la sesión ya se guardó (y la playlist sigue
                        // existiendo), el botón pasa a "Abrir playlist" para no
                        // crear duplicados; vuelve a "Guardar como playlist" si
                        // se borró esa playlist (djPrivado.playlist_guardada_id → 0).
                        readonly property bool _guardada: djPrivado.playlist_guardada_id > 0
                        height: 28
                        ...
                        color: btn_save_ma.containsMouse
                            ...
                            : (_guardada ? Qt.rgba(raiz.tema.acento.r, raiz.tema.acento.g, raiz.tema.acento.b, 0.08)
                                         : raiz.tema.superficieAlt)
                        border.color: _guardada ? raiz.tema.acento : raiz.tema.borde; border.width: 1
                        AppText {
                            id: btn_save_lbl; anchors.centerIn: parent
                            text: parent._guardada ? "Abrir playlist" : "Guardar como playlist"
                            color: parent._guardada ? raiz.tema.acento : raiz.tema.textoSec
                            font.pixelSize: UiTokens.fontSizeSm; font.weight: Font.DemiBold
                        }
                        MouseArea {
                            id: btn_save_ma; anchors.fill: parent; hoverEnabled: true
                            cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                if (parent._guardada && raiz.shell && raiz.shell.abrir_playlist_desde_inicio)
                                    raiz.shell.abrir_playlist_desde_inicio(djPrivado.playlist_guardada_id)
                                else
                                    raiz.abrirGuardar()
                            }
                        }
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        RowLayout {
                            ...
                            StatusBadge {
                                tema: raiz.tema
                                visible: !!(fila.datos && (fila.datos.estado === "reproducida" || fila.datos.estado === "saltada"))
                                text: fila.datos
                                    ? (fila.datos.estado === "reproducida" ? "escuchada"
                                       : (fila.datos.estado === "saltada" ? "omitida" : ""))
                                    : ""
                                tone: fila.datos && fila.datos.estado === "reproducida" ? "success"
                                    : (fila.datos && fila.datos.estado === "saltada" ? "warning" : "neutral")
                                compact: true
                                maxTextWidth: 80
                            }
                            // Botones "Fijar/Soltar" y "Regenerar desde aquí"
                            // eliminados: el reordenamiento manual por pista no
                            // se reflejaba de forma fiable. La sesión se
                            // construye/extiende automáticamente y se gestiona
                            // como un todo (Guardar/Abrir playlist).
                        }
                    }
