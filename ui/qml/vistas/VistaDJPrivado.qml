...
Rectangle {
    ...
    onVisibleChanged: {
        // Al volver a la vista, revalidar si la playlist guardada sigue
        // existiendo: si el usuario la borró desde Playlists, el botón
        // "Guardar como playlist" debe reaparecer.
        if (visible)
            djPrivado.revalidarPlaylistGuardada()
    }
    ...
    Popup {
        ...
        // Nombre por defecto "DJ Privado: Music Session, Vol. X" (el resumen del
        // prompt va a la descripción, no al título). Se recalcula al abrir.
        property string nombreSugerido: djPrivado.nombreSugeridoPlaylist()
        onOpened: input_nombre.text = djPrivado.nombreSugeridoPlaylist()
