...
class ModeloImportacion(QObject):
    ...
    def _calcular_resumen_historial(self, filas: list[dict]) -> dict:
        ...
        for fila in filas:
            ...
            resumen["total_revision"] += revision
            resumen["total_cuarentena"] += cuarentena
        ...
        try:
            ...
            resumen["total_pendientes"] = (
                self._entero_no_negativo(actuales.get("revision"))
                + self._entero_no_negativo(actuales.get("cuarentena"))
            )
        except Exception as exc:
...
class ModeloKaraoke(QObject):
    ...
    def cancelar_procesamiento(self) -> None:
        ...
        snap.update({
            ...
            "mensaje": "Cancelando procesamiento...",
        })
...
class ModeloDjPrivado(QObject):
    ...
    def dj_siguiente(self) -> None:
        if self._reproductor_sesion is not None:
            self._reproductor_sesion.siguiente()

    @Slot()
    def dj_anterior(self) -> None:
        if self._reproductor_sesion is not None:
            self._reproductor_sesion.anterior()

    @Slot(int)
    def dj_saltar_a(self, indice: int) -> None:
        if self._reproductor_sesion is not None:
            self._reproductor_sesion.saltar_a(int(indice))
    ...
    def dj_buscar_global(self, seg_global: float) -> None:
        ...
        """
        if self._reproductor_sesion is not None:
            self._reproductor_sesion.buscar_posicion_global(float(seg_global))

    @Slot(int)
    def dj_set_volumen(self, valor: int) -> None:
        if self._reproductor_sesion is not None:
            self._reproductor_sesion.set_volumen(int(valor))
    ...
    @Slot(int)
    def bloquear_posicion(self, posicion: int) -> None:
        if not self._sesion_id_actual or not self._servicio:
            return
        self._servicio.bloquear_pista(posicion, True)
        self._recargar_pistas_planificadas()

    @Slot(int)
    def desbloquear_posicion(self, posicion: int) -> None:
        if not self._sesion_id_actual or not self._servicio:
            return
        self._servicio.bloquear_pista(posicion, False)
        self._recargar_pistas_planificadas()
    ...
    @Slot(int)
    def replanificar_desde(self, posicion: int) -> None:
        if not self._sesion_id_actual or not self._servicio:
            return
        try:
            self._servicio.replanificar_desde(int(posicion))
            self._recargar_pistas_planificadas()
            self.avisoUi.emit("Sesión replanificada.", "info")
        except Exception as e:
            _log.error(f"[dj] replanificar falló: {e}", exc_info=True)
            self.error.emit("No se pudo replanificar la sesión.")

    @Slot(int)
    def extender(self, minutos_extra: int) -> None:
        if not self._sesion_id_actual or not self._servicio:
            return
        if minutos_extra <= 0:
            return
        try:
            self._servicio.extender_sesion(int(minutos_extra))
            self._recargar_pistas_planificadas()
            self.avisoUi.emit(f"Sesión extendida +{int(minutos_extra)}min.", "info")
        except Exception as e:
            _log.error(f"[dj] extender falló: {e}", exc_info=True)
            self.error.emit("No se pudo extender la sesión.")
    ...
    def guardar_como_playlist(self, nombre: str) -> int:
        if not self._sesion_id_actual or not self._servicio:
            ...
            return 0
        try:
            playlist_id = self._servicio.guardar_como_playlist(nombre.strip() or "Sesión DJ")
