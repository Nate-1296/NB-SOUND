...
def write_windows_version_file(root: Path) -> str:
    """Genera el recurso VS_VERSION_INFO del .exe de Windows desde infra.version.

    Embeber estos metadatos hace que el Explorador, el Administrador de tareas y
    el diálogo UAC muestren un desarrollador y un nombre de producto ("NB Sound"
    / "Nathan") en lugar de quedar en blanco — el "editor desconocido" que el
    usuario reportaba al instalar. (La advertencia de SmartScreen de "editor
    desconocido" solo desaparece firmando con Authenticode; ver build.ps1, que
    ya admite una firma opcional cuando hay certificado.)

    Devuelve la ruta al archivo de version-info que consume ``EXE(version=...)``.
    """
    import re
    from importlib import import_module

    ver = import_module("infra.version")
    numeros = re.findall(r"\d+", str(getattr(ver, "APP_VERSION", "0")))[:4]
    while len(numeros) < 4:
        numeros.append("0")
    cuaterna = tuple(int(n) for n in numeros)
    autor = getattr(ver, "APP_AUTHOR", "NB Sound")
    descripcion = getattr(ver, "APP_DESCRIPTION", "NB Sound")
    licencia = getattr(ver, "APP_LICENSE", "")
    version_txt = str(getattr(ver, "APP_VERSION", "0"))
    copyright_txt = f"(C) {autor}" + (f" - {licencia}" if licencia else "")

    contenido = f"""# Generado por _common.write_windows_version_file — NO editar a mano.
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers={cuaterna},
    prodvers={cuaterna},
    ...
    StringFileInfo([
      StringTable(
        ...
        [
          ...
          StringStruct('FileVersion', {version_txt!r}),
          StringStruct('InternalName', 'nb_sound'),
          StringStruct('LegalCopyright', {copyright_txt!r}),
          StringStruct('OriginalFilename', 'nb_sound.exe'),
          StringStruct('ProductName', 'NB Sound'),
          StringStruct('ProductVersion', {version_txt!r}),
        ])
    ]),
...
"""
    ...
    ruta = destino / "windows_version_info.txt"
