...
def test_vista_importacion_simple_y_pro_con_historial_y_filtros_revision():
    ...
    assert 'Dry-run prueba la importación sin aplicarla' in qml
...
def test_modelo_importacion_expone_resumen_historial_acumulado(db_tmp):
    ...
    # `total_revision`/`total_cuarentena` son los contadores VIVOS (tabla de
    # pendientes, consultada vía `contar_pendientes()`); como el test solo
    # inserta en `sesiones_import`, valen 0. El acumulado histórico de cada
    # sesión vive en `total_revision_historico`/`total_cuarentena_historico`.
    assert modelo.resumen_historial == {
        "total_descubiertos": 18,
        "total_aceptados": 9,
        "total_revision": 0,
        "total_cuarentena": 0,
        "total_revision_historico": 4,
        "total_cuarentena_historico": 2,
        "total_duplicados": 2,
        "total_errores": 1,
        "total_pendientes": 0,
