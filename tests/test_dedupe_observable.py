...
def test_observable_sin_portada_si_resuelve_por_transitividad(_db, tmp_path):
    """Sin portada, dos pistas con mismo título/artista/álbum/duración SÍ se
    consideran duplicadas (transitividad sin mirar hashes ni portada)."""
    art = _crear_artista()
...
# -----------------------------------------------------------------------------
# Cableado UI: el modelo dispara el barrido y refresca en vivo
# -----------------------------------------------------------------------------

def test_modelo_biblioteca_ejecuta_dedupe_observable(_db, tmp_path):
    """`ModeloBiblioteca.ejecutar_dedupe_observable` (ruta síncrona, sin loop Qt)
    resuelve los duplicados observables y los oculta de la biblioteca."""
    pytest.importorskip("PySide6")
    from ui.modelos_qml import ModeloBiblioteca

    portada = _crear_portada(tmp_path, "cover.jpg", b"COVER-DATA-001")
    art = _crear_artista("Artista")
    alb = _crear_album(art, "Album", portada)
    # Dos pistas idénticas en título/artista/álbum/portada/duración: duplicado
    # observable que la 3a capa debe resolver, dejando una sola en biblioteca.
    conservar = _crear_pista(titulo="Tema", album_id=alb, artista_id=art,
                             bitrate_kbps=320, hash_sha256="hA")
    duplicada = _crear_pista(titulo="Tema", album_id=alb, artista_id=art,
                             bitrate_kbps=128, hash_sha256="hB")

    modelo = ModeloBiblioteca()
    spy_fin = []
    modelo.dedupeObservableFinalizado.connect(lambda d: spy_fin.append(d))
    modelo.ejecutar_dedupe_observable()
