...
# ADAPTACION (skip)
#
# La adaptación manual por pista (replanificar desde, extender +15 min, fijar)
# se eliminó de la app: no se reflejaba de forma fiable y la cola ya se extiende
# sola en background (continuar_construccion). Solo queda el registro de "skip",
# que alimenta las exclusiones del constructor.
# =============================================================================
...
def test_nombre_auto_playlist_incrementa_sin_repetir(db_dj):
    """El nombre por defecto al guardar una sesión DJ es
    "DJ Privado: Music Session, Vol. N", con N incremental sin repetir."""
    from servicios import biblioteca as svc_bib

    assert DjPrivadoService._nombre_auto_playlist() == "DJ Privado: Music Session, Vol. 1"

    svc_bib.crear_playlist("DJ Privado: Music Session, Vol. 1")
    svc_bib.crear_playlist("DJ Privado: Music Session, Vol. 2")
    assert DjPrivadoService._nombre_auto_playlist() == "DJ Privado: Music Session, Vol. 3"

    # Huecos: respeta el máximo, no rellena.
    svc_bib.crear_playlist("DJ Privado: Music Session, Vol. 7")
    assert DjPrivadoService._nombre_auto_playlist() == "DJ Privado: Music Session, Vol. 8"
