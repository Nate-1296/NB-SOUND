...
def test_modelo_playlists_asegurar_portadas_async_refresca_si_hay_cambios(monkeypatch):
    """asegurar_portadas_async corre vía worker; si regenera carátulas refresca
    la lista de Playlists y emite portadasAseguradas (para refrescar Inicio)."""
    pytest.importorskip("PySide6")
    from PySide6.QtGui import QGuiApplication
    _app = QGuiApplication.instance() or QGuiApplication([])
    ...
    llamadas: list[str] = []
    monkeypatch.setattr(
        modelos_qml.svc_bib, "asegurar_portadas_playlists",
        lambda: llamadas.append("asegurar") or [7, 9],
    )
    monkeypatch.setattr(
        modelos_qml.svc_bib, "listar_playlists",
        lambda *a, **k: llamadas.append("listar") or [],
    ...
    modelo = modelos_qml.ModeloPlaylists()
    emitido: list = []
    modelo.portadasAseguradas.connect(lambda ids: emitido.append(ids))

    modelo.asegurar_portadas_async()

    assert "asegurar" in llamadas
    assert "listar" in llamadas  # cargar() refrescó la lista de Playlists
    assert emitido == [[7, 9]]


def test_modelo_playlists_asegurar_portadas_async_sin_cambios_no_refresca(monkeypatch):
    """Si el barrido no regenera nada, no recarga la lista ni emite señal."""
    pytest.importorskip("PySide6")
    from PySide6.QtGui import QGuiApplication
    _app = QGuiApplication.instance() or QGuiApplication([])
    ...
    monkeypatch.setattr(
        modelos_qml.svc_bib, "asegurar_portadas_playlists",
        lambda: llamadas.append("asegurar") or [],
    )
    monkeypatch.setattr(
        modelos_qml.svc_bib, "listar_playlists",
        lambda *a, **k: llamadas.append("listar") or [],
    ...
    assert "asegurar" in llamadas
    assert "listar" not in llamadas  # sin cambios: no recarga
    assert emitido == []
