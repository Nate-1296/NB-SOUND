...
def test_genera_this_is_y_moods_solo_con_datos_suficientes(db_playlists, monkeypatch):
    ...
    # 24 pistas (sin género) quedan por debajo del mínimo de 25 de "This is".
    ids = _crear_lote(db_playlists, 24, artista="Adele", album="Live", reproducciones=False)
    svc_bib.sincronizar_playlists_sistema(5)
    ...
    # Al llegar a 30 supera el mínimo de "This is" (25) y habilita moods (30).
    for i in range(6):
        ids.append(_crear_pista(db_playlists, f"Extra {i:02d}", artista="Adele", album="Live"))
    svc_bib.sincronizar_playlists_sistema(5)
...
def test_this_is_crea_con_min_pistas_y_expone_conteos(db_playlists):
    # 24 pistas (sin género, sin similares) no alcanzan el mínimo de 25.
    _crear_lote(db_playlists, 24, artista="Artista 25", album="Directos")
    svc_bib.sincronizar_playlists_sistema(5)
    assert not any(pl["auto_key"] for pl in svc_bib.listar_playlists() if pl["auto_key"] == "auto:this_is:1")

    _crear_pista(db_playlists, "Cancion 25", artista="Artista 25", album="Directos")
    svc_bib.sincronizar_playlists_sistema(0)

    conteo = next(item for item in svc_bib.conteos_artistas_para_playlists() if item["nombre"] == "Artista 25")
    assert conteo["total_pistas"] == 25
    assert {"artista_id", "nombre", "total_pistas", "total_favoritas", "reproducciones_total", "ultima_actualizacion"} <= set(conteo)
    assert any(pl["tipo_playlist"] == "this_is" and pl["nombre"] == "This is Artista 25" for pl in svc_bib.listar_playlists())


def test_this_is_supera_50_y_rellena_con_similares(db_playlists):
    """'This is' admite hasta 80 (tope por-spec, no el global de 50) y completa
    con temas similares por género cuando el artista tiene pocas pistas."""
    # Artista con 62 pistas propias: la playlist debe superar el viejo tope 50.
    _crear_lote(db_playlists, 62, artista="Prolífico", album="Obra", reproducciones=False)
    svc_bib.sincronizar_playlists_sistema(0)
    detalle = next(pl for pl in svc_bib.listar_playlists() if pl["nombre"] == "This is Prolífico")
    completo = svc_bib.detalle_playlist(detalle["playlist_id"])
    assert 50 < completo["num_pistas"] <= 80

    # Artista con solo 22 pistas pero género compartido: se rellena con
    # similares (otros artistas del mismo género) hasta alcanzar el mínimo.
    for i in range(22):
        _crear_pista(db_playlists, f"Corto {i:02d}", artista="Corto", album="EP", genero="indie")
    # Vecindario del mismo género que aporta el relleno.
    for i in range(20):
        _crear_pista(db_playlists, f"Vecino {i:02d}", artista=f"Vecino {i % 5}", album="V", genero="indie")
    svc_bib.sincronizar_playlists_sistema(0)
    corto = next((pl for pl in svc_bib.listar_playlists() if pl["nombre"] == "This is Corto"), None)
    assert corto is not None, "Debe crearse 'This is Corto' rellenando con similares"
    detalle_corto = svc_bib.detalle_playlist(corto["playlist_id"])
    assert detalle_corto["num_pistas"] >= 25
...
def test_asegurar_portadas_playlists_regenera_las_que_faltan_y_es_idempotente(
    db_playlists, tmp_path, monkeypatch
):
    Image = pytest.importorskip("PIL.Image")
    from config import settings as _settings
    ...
    def _portada_db(playlist_id: int) -> str:
        fila = get_conexion().execute(
            "SELECT portada_ruta FROM playlists WHERE id = ?", (int(playlist_id),)
        ).fetchone()
        return str(fila["portada_ruta"] or "")

    # Playlist al día: poblada por las vías normales, ya tiene carátula generada.
    al_dia = svc_bib.crear_playlist("Al día")
    for pista_id in ids:
        svc_bib.agregar_a_playlist(al_dia, pista_id)
    assert Path(_portada_db(al_dia)).exists()

    # Playlist sin carátula "hecha": simulamos el estado problemático borrando la
    # portada en BD (creada antes de la feature o con el archivo perdido).
    sin_caratula = svc_bib.crear_playlist("Sin carátula")
    for pista_id in ids:
        svc_bib.agregar_a_playlist(sin_caratula, pista_id)
    get_conexion().execute(
        "UPDATE playlists SET portada_ruta = '' WHERE id = ?", (int(sin_caratula),)
    )

    actualizadas = svc_bib.asegurar_portadas_playlists()

    # Solo reporta la que faltaba; la que ya estaba al día no se toca (idempotente).
    assert sin_caratula in actualizadas
    assert al_dia not in actualizadas

    portada_path = Path(_portada_db(sin_caratula))
    assert portada_path.exists()
    assert (tmp_path / "cache" / "playlist_covers") in portada_path.parents

    # Un segundo barrido no encuentra nada que regenerar.
...
def test_playlists_editables_para_pista_marca_pertenencia(db_playlists):
    """El selector 'agregar a playlist' solo lista manuales y pre-marca
    correctamente las que ya contienen la pista."""
    pista_a = _crear_pista(db_playlists, "Cancion A", artista="Tester", album="Suite")
    pista_b = _crear_pista(db_playlists, "Cancion B", artista="Tester", album="Suite")

    pl_con = svc_bib.crear_playlist("Con la pista")
    pl_sin = svc_bib.crear_playlist("Sin la pista")
    svc_bib.agregar_a_playlist(pl_con, pista_a)

    # Crea la playlist de favoritos (sistema) marcando una favorita y listando.
    get_conexion().execute("UPDATE pistas SET favorita = 1 WHERE id = ?", (pista_b,))
    svc_bib.listar_playlists()  # dispara _sincronizar_playlist_favoritos

    items = svc_bib.playlists_editables_para_pista(pista_a)
    por_id = {it["playlist_id"]: it for it in items}

    # Solo manuales: favoritos/sistema/automáticas quedan fuera.
    nombres = {it["nombre"] for it in items}
    assert "Me gusta" not in nombres
    assert pl_con in por_id and pl_sin in por_id
    assert por_id[pl_con]["contiene"] is True
    assert por_id[pl_sin]["contiene"] is False
    assert por_id[pl_con]["num_pistas"] == 1


def test_modelo_aplicar_en_playlists_diff(db_playlists):
    """`aplicar_en_playlists` agrega/quita en lote según el diff."""
    pytest.importorskip("PySide6")
    from ui.modelos_qml import ModeloPlaylists

    pista_id = _crear_pista(db_playlists, "Cancion", artista="Tester", album="Suite")
    pl_1 = svc_bib.crear_playlist("Lista 1")
    pl_2 = svc_bib.crear_playlist("Lista 2")
    svc_bib.agregar_a_playlist(pl_2, pista_id)  # ya pertenece a pl_2

    modelo = ModeloPlaylists()

    # Estado de partida visible para el selector.
    items = modelo.playlists_para_pista(pista_id)
    por_id = {it["playlist_id"]: it for it in items}
    assert por_id[pl_1]["contiene"] is False
    assert por_id[pl_2]["contiene"] is True

    # Diff: agregar a pl_1, quitar de pl_2.
    resultado = modelo.aplicar_en_playlists(pista_id, [pl_1], [pl_2])
    assert resultado["ok"] is True
    assert resultado["agregadas"] == 1
    assert resultado["quitadas"] == 1

    items2 = modelo.playlists_para_pista(pista_id)
    por_id2 = {it["playlist_id"]: it for it in items2}
    assert por_id2[pl_1]["contiene"] is True
    assert por_id2[pl_2]["contiene"] is False

    # Crear playlist desde el selector no cambia la activa.
    creada = modelo.crear_playlist_para_seleccion("Nueva desde selector")
    assert creada["ok"] is True
    assert modelo.playlist_activa_id == -1  # nunca se abrió ninguna


def test_playlist_existe(db_playlists):
    """`playlist_existe` valida existencia y visibilidad (para el botón de
    guardar del DJ: si la playlist se borra, debe reaparecer)."""
    pl = svc_bib.crear_playlist("Para DJ")
    assert svc_bib.playlist_existe(pl) is True
    assert svc_bib.playlist_existe(0) is False
    assert svc_bib.playlist_existe(99999) is False
    # Tras eliminar (manual sin auto_key → borrado real), deja de existir.
    svc_bib.eliminar_playlist(pl)
    assert svc_bib.playlist_existe(pl) is False
