...
def test_resumen_karaoke_total_coincide_con_suma_estados(db_karaoke):
    """El 'Total' debe ser SIEMPRE igual a la suma de las pestañas, incluso si
    alguna pista quedó con un karaoke_estado desconocido/legacy (se normaliza a
    'no_procesada' en vez de inflar el total como un fantasma invisible)."""
    _crear_pista(db_karaoke, "ok1", estado_karaoke="lista")
    _crear_pista(db_karaoke, "raro", estado_karaoke="preparando")  # estado no válido
    r = svc_bib.resumen_karaoke()
    suma_estados = (
        r["no_procesada"] + r["en_cola"] + r["procesando"]
        + r["lista"] + r["fallida"] + r["no_aplica"]
    )
    assert r["total"] == suma_estados == 2
    assert r["sin_preparar"] == 1  # la pista con estado raro cae aquí
...
def test_cancelar_pista_en_cola(db_karaoke):
    d = _crear_pista(db_karaoke, "cp_cola")
    svc_kar.encolar(d["id"])
    assert svc_kar.cancelar_pista(d["id"]) is True
    p = svc_bib.pista_karaoke_por_id(d["id"])
    assert p["karaoke_estado"] == "no_procesada"


def test_cancelar_pista_procesando(db_karaoke):
    d = _crear_pista(db_karaoke, "cp_proc")
    jid = svc_kar.encolar(d["id"])
    svc_kar.jobs_repo.transicionar_estado(jid, "procesando", progreso=0.4)
    assert svc_kar.cancelar_pista(d["id"]) is True
    p = svc_bib.pista_karaoke_por_id(d["id"])
    assert p["karaoke_estado"] == "no_procesada"
    job = svc_kar.jobs_repo.job_por_id(jid)
    assert job["estado"] == "cancelada"


def test_cancelar_pista_reconcilia_procesando_falso(db_karaoke):
    """'Procesando falso': la pista quedó en 'procesando' pero sin job vivo
    (p.ej. tras un cierre forzado). El cancelar robusto debe liberarla igual."""
    d = _crear_pista(db_karaoke, "cp_falso", estado_karaoke="procesando")
    # No hay job activo asociado: solo el cache colgado.
    assert svc_kar.cancelar_pista(d["id"]) is True
    p = svc_bib.pista_karaoke_por_id(d["id"])
    assert p["karaoke_estado"] == "no_procesada"


def test_cancelar_pista_sin_nada_que_cancelar(db_karaoke):
    d = _crear_pista(db_karaoke, "cp_noop")  # no_procesada, sin job
    assert svc_kar.cancelar_pista(d["id"]) is False
...
def test_qml_acciones_por_estado():
    ...
    # en_cola y procesando usan el cancelar robusto por pista.
    assert "cancelar_pista" in qml
