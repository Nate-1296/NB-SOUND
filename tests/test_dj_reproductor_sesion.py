...
def test_modeloreproductor_icono_pausa_al_ceder_audio_al_dj(db_dj):
    """#6: cuando el DJ toma el audio, el reproductor global debe mostrarse como
    NO reproduciendo (icono play) de inmediato, aunque su estado lógico siga en
    REPRODUCIENDO. Así el botón refleja que el global quedó en pausa."""
    pytest.importorskip("PySide6")
    from PySide6.QtGui import QGuiApplication
    app = QGuiApplication.instance() or QGuiApplication([])
    from servicios.reproductor import Reproductor, EstadoReproductor
    from ui.modelos_qml import ModeloReproductor
    r = Reproductor(permitir_modo_simulado=True)
    m = ModeloReproductor(r)
    # Mimetiza "global sonando".
    r._estado = EstadoReproductor.REPRODUCIENDO
    assert m.reproduciendo is True
    assert m.pausado is False
    # El DJ toma ownership: el global se suspende.
    r.set_modo_dj(True)
    app.processEvents()
    assert m.reproduciendo is False   # icono → play (continuar)
    assert m.pausado is True
    # Al devolver el audio al global, vuelve a reflejar su estado real.
    r.set_modo_dj(False)
    app.processEvents()
    # set_modo_dj(False) deja el global en PAUSADO (silencio predecible).
    assert m.reproduciendo is False
