...
def test_background_retry_failed_reintenta_jobs_con_reintentos_agotados(tmp_path: Path):
    """El reintento MANUAL debe reencolar también jobs fallidos que ya agotaron
    sus reintentos automáticos (attempts >= max_attempts), reseteando attempts."""
    inicializar_db(tmp_path / "retry_exhausted.sqlite")
    model_dir = _configure(tmp_path)
    _insert_track(1, "/music/a.mp3", "h1")
    get_conexion().execute(
        "INSERT INTO audio_analysis_runs(run_id, mode, status, started_at) VALUES(?,?, 'completed', datetime('now'))",
    ...
    # attempts == max_attempts (2): antes quedaba fuera del reintento.
    get_conexion().execute(
        """
        INSERT INTO audio_analysis_jobs(job_id, run_id, track_id, job_type, status, attempts, max_attempts, model_version, file_hash)
        VALUES('job-exhausted', 'run-old', '1', 'deep', 'failed', 2, 2, ?, 'h1')
        """,
        (DEEP_ANALYZER_VERSION,),
    )
    ...
    svc = _service(tmp_path, model_dir)
    retry = svc.retry_failed()
    assert retry.get("requeued") == 1

    job = get_conexion().execute(
        "SELECT status, attempts FROM audio_analysis_jobs WHERE job_id='job-exhausted'"
    ).fetchone()
    assert job["status"] == "pending"
    assert job["attempts"] == 0  # reseteado para que vuelva a ejecutarse
