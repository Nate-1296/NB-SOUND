...
def test_desactivar_aleatorio_restaura_orden_base_completo(db_reproductor, monkeypatch):
    monkeypatch.setattr(reproductor_mod.random, "shuffle", lambda items: items.reverse())
    pistas = [
        _crear_pista(db_reproductor, "shuffle-base-a"),
        _crear_pista(db_reproductor, "shuffle-base-b"),
        _crear_pista(db_reproductor, "shuffle-base-c"),
        _crear_pista(db_reproductor, "shuffle-base-d"),
    ]
    rep = _crear_reproductor()
    ...
    # La cola no es consumible: al desactivar el aleatorio se restaura el orden
    # base COMPLETO (ninguna pista se pierde). El puntero queda sobre la pista
    # activa: el fin de pista avanzó de la 2.ª (b) a la 4.ª (d) dentro del orden
    # barajado [b, d, c, a], y al restaurar la base d queda en el índice 3.
    assert [p["titulo"] for p in rep.obtener_cola()] == [
        pistas[0]["titulo"],
        pistas[1]["titulo"],
        pistas[2]["titulo"],
...
def test_sorprenderme_diversifica_artistas_y_albums_con_alternativas(db_reproductor, monkeypatch):
    ...
    import ui.modelos_qml as modelos_qml
    # `random.random` se anula para eliminar el jitter del score (orden de ranking
    # determinista). Pero la elección final usa `random.choices`, que internamente
    # usa el método del Random global (no el `random.random` del módulo): por eso
    # parchearlo no bastaba y el test era flaky según el orden de ejecución de la
    # suite. Forzamos `choices` al candidato de mayor peso (primer elemento del
    # top), que es exactamente lo que este test valida sobre la diversificación,
    # sin alterar el comportamiento de `sorprenderme()`.
    monkeypatch.setattr(modelos_qml.random, "random", lambda: 0.0)
    monkeypatch.setattr(
        modelos_qml.random,
        "choices",
        lambda population, weights=None, k=1: list(population[:k]),
    )
...
def test_repetir_todo_cicla_al_terminar_ultima_pista(db_reproductor):
    ...
    # Con repetir-todo, al terminar la última el puntero vuelve al inicio sin
    # alterar la cola (no consumible).
    assert [p["titulo"] for p in rep.obtener_cola()] == [
        pista_a["titulo"],
        pista_b["titulo"],
    ]
...
def test_repetir_todo_contexto_respeta_reorden_manual(db_reproductor):
    ...
    # El reorden manual [c, a, b] se conserva (cola no consumible). Partiendo de
    # la activa b (índice 2), tres avances con repetir-todo dan un ciclo completo
    # 2 -> 0 -> 1 -> 2, devolviendo el puntero a b sin alterar el orden.
    assert [p["titulo"] for p in rep.obtener_cola()] == [
        pista_c["titulo"],
        pista_a["titulo"],
        pista_b["titulo"],
...
def test_reproductor_avanza_en_cola_al_finalizar_sin_consumir(db_reproductor):
    ...
    # La cola no es consumible: al terminar la 1.ª, solo avanza el puntero; ambas
    # pistas permanecen (igual que `siguiente`).
    assert [p["titulo"] for p in rep.obtener_cola()] == [
        pista_a["titulo"],
        pista_b["titulo"],
    ]
...
def test_reproductor_detiene_conservando_pista_al_terminar_ultima(db_reproductor):
    ...
    # `reproducir_pista` descarta la cola previa (queda solo la individual). Al
    # terminar la última pista sin repetición, la reproducción se detiene pero
    # la pista y la cola se conservan, con el puntero en ella: sigue marcada
    # como actual (indice_cola válido) y `play` la reanuda.
    assert [p["titulo"] for p in rep.obtener_cola()] == [individual["titulo"]]
    assert rep.indice_cola == 0
    assert rep.pista_activa is not None
    assert rep.pista_activa.titulo == individual["titulo"]
    assert rep.estado == EstadoReproductor.FINALIZADA


def test_reproductor_avanza_a_pista_anadida_conservando_individual(db_reproductor):
    ...
    # La individual no se consume: queda al inicio de la cola y el puntero avanza
    # a la primera pista añadida (a).
    assert [p["titulo"] for p in rep.obtener_cola()] == [
        individual["titulo"],
        pista_a["titulo"],
        pista_b["titulo"],
...
def test_qml_biblioteca_fase5_ux_acciones_claras_sin_deprecated():
    ...
    # Ancho de la columna "Acciones" del header = el de las filas (348) para que
    # "Duración"/"Acciones" queden alineadas con sus valores.
    assert "Layout.preferredWidth: 348" in album
