...
def test_qml_personalizacion_tema_fuente_y_escala_en_vivo(qml_factory):
    ...
    assert len([t for t in tema.temas_disponibles if t["id"] != "custom"]) == 63
...
def test_qml_personalizacion_ecualizador_existe_y_se_guarda(qml_factory):
    """Smoke del ítem 7: la sección de personalización monta el ecualizador del
    reproductor global (presets + 10 bandas + preamp + normvol) sin errores QML,
    y los controles persisten al instante en config_ui."""
    runtime = qml_factory("personalizacion_eq.db")
    vista = _abrir_configuracion(runtime)
    vista.setProperty("seccion_activa", "personalizacion")
    _wait(runtime.app, 120)

    reproductor = runtime.modelos["reproductor"]

    # El grupo y los controles "sueltos" del ecualizador existen (la vista los
    # montó sin errores QML). Los chips de preset y las 10 bandas se generan con
    # Repeater: sus delegados no son alcanzables por findChild/objectName, así
    # que se validan por el `count` del modelo y el flujo funcional, no por _find.
    assert _find(vista, "config_personalizacion_ecualizador") is not None
    assert _find(vista, "config_eq_activo") is not None
    assert _find(vista, "config_eq_preset_custom") is not None
    assert _find(vista, "config_eq_preamp") is not None
    assert _find(vista, "config_audio_normalizar") is not None

    # Metadatos expuestos a QML: 18 presets + 10 bandas (alimentan los Repeater).
    assert len(list(reproductor.eq_presets_nombres)) == 18
    assert list(reproductor.eq_bandas_hz) == [31, 62, 125, 250, 500, 1000, 2000, 4000, 8000, 16000]

    # Flujo completo: activar, aplicar preset, mover banda (-> Personalizado),
    # normvol. Todo se guarda al instante en config_ui.
    reproductor.set_ecualizador_activo(True)
    reproductor.aplicar_ecualizador_preset(13)   # Rock
    assert reproductor.eq_preset == 13
    reproductor.set_ecualizador_banda(0, 4.0)
    assert reproductor.eq_preset == -1
    assert list(reproductor.eq_bandas)[0] == pytest.approx(4.0)
    reproductor.set_normalizar_volumen(True)
    _wait(runtime.app, 80)
    ...
    # El indicador "Personalizado" (chip suelto, no del Repeater) refleja el
    # estado custom en vivo vía binding.
    assert _find(vista, "config_eq_preset_custom").property("activo") is True
